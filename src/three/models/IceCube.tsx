...
export function IceCube({ position, scale = 1, seed = 1, reduced = false, background = "#ffd6df", simple = false }: {
  position: [number, number, number]; scale?: number; seed?: number; reduced?: boolean; background?: string; simple?: boolean;
}) {
  ...
  return <group ref={ref} position={position} scale={scale} rotation={[.4, seed * .8, .3]}>
    <RoundedBox args={[.74, .81, .72]} radius={.115} smoothness={3}>
      {simple ? <meshPhysicalMaterial color="#eefbff" transmission={.96} thickness={.6}
        transparent opacity={.62} ior={1.31} roughness={.08} clearcoat={.35} envMapIntensity={.4} /> :
        <MeshTransmissionMaterial background={backdrop} resolution={128} samples={3}
          color="#edfaff" transmission={1} thickness={.65} ior={1.31} roughness={.07}
          chromaticAberration={.015} distortion={.12} distortionScale={.25} temporalDistortion={0}
          clearcoat={.3} clearcoatRoughness={.16} envMapIntensity={.45} />}
    </RoundedBox>
    <RoundedBox args={[.43, .48, .41]} radius={.12} smoothness={2} rotation={[.1, .2, .14]}>
      <meshPhysicalMaterial color="#e8faff" roughness={.72} transparent opacity={.09} depthWrite={false} />
    </RoundedBox>
    {pockets.map((p, i) => <mesh key={i} position={[p[0], p[1], p[2]]} scale={[p[3], p[3] * 1.5, p[3]]}>
      <sphereGeometry args={[1, 8, 6]} />
      <meshPhysicalMaterial color="white" roughness={.12} transparent opacity={.52} depthWrite={false} />
    </mesh>)}
